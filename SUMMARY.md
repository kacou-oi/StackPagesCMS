# Table des Matières

* [Introduction et Fonctionnalités](README.md)

* [1. Architecture et API](docs/architecture/README.md)
    * [1.1 Flux de Données et Composants](docs/architecture/architecture-flow.md)
    * [1.2 Détail des Endpoints API](docs/architecture/endpoints.md)

* [2. Guide de Déploiement](docs/deploiement/README.md)
    * [2.1 Configuration du Worker Cloudflare](docs/deploiement/cloudflare-setup.md)
    * [2.2 Configuration du Frontend Webstudio](docs/deploiement/webstudio-setup.md)

* [3. Maintenance et Dépannage](docs/maintenance/README.md)
    * [3.1 Purge du Cache](docs/maintenance/purge-cache.md)

* [4. Licence et Contribution](LICENSE)
